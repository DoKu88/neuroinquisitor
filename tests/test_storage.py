...
def test_epoch_file_created(obs: NeuroInquisitor, tmp_path: Path) -> None:
    ...
    assert (tmp_path / "epoch_0000.safetensors").exists()
...
def test_multiple_epoch_files_created(obs: NeuroInquisitor, tmp_path: Path) -> None:
    ...
    assert (tmp_path / "epoch_0000.safetensors").exists()
    assert (tmp_path / "epoch_0001.safetensors").exists()
...
# ---------------------------------------------------------------------------
# SnapshotCollection — load_all_snapshots
# ---------------------------------------------------------------------------
...
def test_load_all_snapshots_returns_collection(multi_obs: NeuroInquisitor) -> None:
    col = multi_obs.load_all_snapshots()
    multi_obs.close()
    assert isinstance(col, SnapshotCollection)
    assert len(col) == 4


def test_load_all_snapshots_epochs_property(multi_obs: NeuroInquisitor) -> None:
    col = multi_obs.load_all_snapshots()
    multi_obs.close()
    assert col.epochs == [0, 1, 2, 3]


def test_load_all_snapshots_layers_property(
    mlp: nn.Module, multi_obs: NeuroInquisitor
) -> None:
    col = multi_obs.load_all_snapshots()
    multi_obs.close()
...
def test_by_epoch_raises_for_missing(multi_obs: NeuroInquisitor) -> None:
    col = multi_obs.load_all_snapshots()
    multi_obs.close()
    with pytest.raises(KeyError, match="epoch 99"):
        col.by_epoch(99)
...
def test_by_layer_returns_all_epochs(multi_obs: NeuroInquisitor) -> None:
    col = multi_obs.load_all_snapshots()
    multi_obs.close()
...
def test_by_layer_reads_in_parallel(multi_obs: NeuroInquisitor) -> None:
    col = multi_obs.load_all_snapshots()
    multi_obs.close()
    # Just verify correctness; parallelism is an implementation detail.
    result = col.by_layer("0.weight", max_workers=4)
    assert set(result.keys()) == {0, 1, 2, 3}


def test_by_layer_values_differ_across_epochs(multi_obs: NeuroInquisitor) -> None:
    col = multi_obs.load_all_snapshots()
    multi_obs.close()
    result = col.by_layer("0.weight")
...
def test_by_layer_raises_for_missing(multi_obs: NeuroInquisitor) -> None:
    col = multi_obs.load_all_snapshots()
    multi_obs.close()
...
def test_select_by_epoch_range(multi_obs: NeuroInquisitor) -> None:
    col = multi_obs.load_all_snapshots()
    multi_obs.close()
...
def test_select_by_epoch_list(multi_obs: NeuroInquisitor) -> None:
    col = multi_obs.load_all_snapshots()
    multi_obs.close()
...
def test_select_by_single_epoch(multi_obs: NeuroInquisitor) -> None:
    col = multi_obs.load_all_snapshots()
    multi_obs.close()
...
def test_select_by_layers(mlp: nn.Module, multi_obs: NeuroInquisitor) -> None:
    col = multi_obs.load_all_snapshots()
    multi_obs.close()
...
def test_select_by_single_layer(multi_obs: NeuroInquisitor) -> None:
    col = multi_obs.load_all_snapshots()
    multi_obs.close()
...
def test_select_combined_epochs_and_layers(multi_obs: NeuroInquisitor) -> None:
    col = multi_obs.load_all_snapshots()
    multi_obs.close()
...
def test_select_returns_new_collection(multi_obs: NeuroInquisitor) -> None:
    col = multi_obs.load_all_snapshots()
    multi_obs.close()
...
def test_select_is_composable(multi_obs: NeuroInquisitor) -> None:
    col = multi_obs.load_all_snapshots()
    multi_obs.close()
...
def test_collection_repr(multi_obs: NeuroInquisitor) -> None:
    col = multi_obs.load_all_snapshots()
    multi_obs.close()
    r = repr(col)
...
def test_load_all_is_lazy(multi_obs: NeuroInquisitor, tmp_path: Path) -> None:
    """load_all_snapshots must not read tensor files — only the index."""
