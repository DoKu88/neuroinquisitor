...
def generate_visualizations(
    ...
    print(f"  Saved: {make_replay_video(replay_history, replay_modules, test_acc_history, replay_vid_path, fps=fps).name}")
...
def make_weight_video(
    ...
    im_cos = axes[0].imshow(cosine_sims[0], cmap="RdBu_r", vmin=-1, vmax=1, interpolation="nearest")
    ...
    axes[3].set_title(f"Output proj ({out_h}×{out_w}) — fixed bounds", fontsize=8)
...
def precompute_replay_data(
    ...
    """Precompute bar-chart magnitudes and gradient heatmaps for all snapshots.
...
def make_replay_video(
    ...
    """Animate activations, gradient bar charts, and gradient heatmaps across snapshots."""
    ...
    def update(frame: int) -> list:
        ...
        title_text.set_text(f"Replay — Snapshot {frame + 1}  |  Test accuracy: {acc:.1%}")
