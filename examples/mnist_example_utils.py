...
def make_video(
    ...
    delta_rendered = [render_delta_gray(snap["conv1.weight"], weight_0["conv1.weight"], delta_vmax) for snap in weight_history]

    fig, axes = plt.subplots(1, 4, figsize=(20, 4), constrained_layout=True)

    im_conv1 = axes[0].imshow(conv1_rendered[0], cmap="gray", interpolation="nearest")
    axes[0].set_title("Conv1 — grayscale filters (fixed bounds)", fontsize=8)
    axes[0].axis("off")
    ...
    im_delta = axes[3].imshow(delta_rendered[0], cmap="hot", interpolation="nearest")
