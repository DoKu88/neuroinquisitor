...
def _precompute_replay_data(
    ...
    for frame in replay_history:
        ...
        for name in modules:
            ...
            # Bar chart: collapse to per-channel scalar
            e_grad[name] = np.abs(grad).reshape(grad.shape[0], -1).mean(axis=1)

            # Heatmap: preserve 2-D spatial (or near-square reshape for FC)
            if grad.ndim == 3:
                # Conv: mean absolute gradient over channels → (H, W)
                e_heat[name] = np.abs(grad).mean(axis=0)
            else:
                # FC: reshape 1-D gradient vector into nearest square 2-D image
...
def _make_replay_video(
    ...
    act_mags, grad_mags, grad_heat = _precompute_replay_data(replay_history, modules)

    # Fixed limits computed across all epochs
...
def _make_combined_video(
    ...
    # ── Weight panel precomputation ──────────────────────────────────────────
    conv1_lims     = _compute_conv1_filter_lims(weight_history)
    conv2_hmaps    = [_render_conv2_norm_heatmap(s["conv2.weight"]) for s in weight_history]
    ...
    conv1_rendered = [_render_rgb_filters(s["conv1.weight"], conv1_lims) for s in weight_history]
    delta_rendered = [_render_delta_rgb(s["conv1.weight"], weight_0["conv1.weight"], delta_vmax) for s in weight_history]

    # ── Replay precomputation ────────────────────────────────────────────────
    act_mags, grad_mags, grad_heat = _precompute_replay_data(replay_history, replay_modules)
    ...
    # ── Replay panel artists ─────────────────────────────────────────────────
    act_bars:  list[Any] = []
    grad_bars: list[Any] = []
    heat_ims:  list[Any] = []

    for c, name in enumerate(replay_modules):
        # Row 1 — activation bar chart
...
def main() -> None:
    ...
    print(f"  Replaying {num_epochs} checkpoints on 128 test samples …")
    for epoch in range(num_epochs):
    ...
    replay_result = _make_replay_video(replay_history, replay_modules, accuracy_history, replay_vid_path, fps=4)
    ...
    combined_result = _make_combined_video(weight_history, replay_history, accuracy_history, replay_modules, combined_path, fps=4)
