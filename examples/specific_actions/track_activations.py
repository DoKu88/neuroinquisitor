...
def main() -> None:
    cfg_path = Path(__file__).parent.parent / "configs" / "specific_actions_track_activations.yaml"
    with open(cfg_path) as f:
        cfg = yaml.safe_load(f)

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_dir = Path(__file__).parent.parent.parent / "outputs" / "track_activations" / timestamp
